...
<div class="p-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="vms-modal-title">{{ __('Assign Task to Team') }}</h2>
    ...
    <form method="POST" action="{{ route('mechanic.tasks.assign', $schedule) }}">
        @csrf
        <div class="mb-3">
            <label class="vms-label">{{ __('Assign to Team Member') }}</label>
            <select name="mechanic_id" class="vms-select" required>
                @foreach ($teamMembers as $m)
                    <option value="{{ $m->id }}" @selected($schedule->assigned_mechanic_id === $m->id)>
                        {{ $m->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="vms-banner-info-blue mb-3">
            ...
            {{ __('Assigning this task will change its status to "In Progress" and log the assigned team member.') }}
        </div>

        <div class="d-flex" style="gap:10px;">
            <button type="submit" class="vms-btn vms-btn-blue flex-grow-1">
                <i class="fas fa-check mr-1"></i> {{ __('Confirm & Assign') }}
            </button>
